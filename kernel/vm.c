...
pte_t *
walk(pagetable_t pagetable, uint64 va, int alloc, int max)
{
  if(va >= MAXVA)
    panic("walk");
...
smappages(pagetable_t pagetable, uint64 va, uint64 size, uint64 pa, int perm)
{
  ...
  if((va % SUPERPGSIZE) != 0)
    panic("smappages: va not aligned");

  if((size % SUPERPGSIZE) != 0)
    panic("smappages: size not aligned");
...
uvmalloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz, int xperm)
{
  ...
  for(a = oldsz; a < newsz; a += sz){
    ...
    int ok;
    if (sz == SUPERPGSIZE) {
      ok = smappages(pagetable, a, sz, (uint64)mem, PTE_R|PTE_U|xperm);
    } else {
      ok = mappages(pagetable, a, sz, (uint64)mem, PTE_R|PTE_U|xperm);
    }
    if(ok != 0){
      kfree(mem);
      uvmdealloc(pagetable, a, oldsz);
